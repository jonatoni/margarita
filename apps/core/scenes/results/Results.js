  ...
  render() {
    ...
    return (
      <SafeAreaView style={styles.container}>
        ...
        />
        <QueryRenderer
          query={getSearchQueryByType(tripType)}
          variables={{
            input: parseSearchParametersByType(tripType, this.props),
          }}
...
const getSearchQueryByType = (type: TripTypes) => {
  let query;
  switch (type) {
    case 'Return':
      query = graphql`
...
const parseSearchParametersByType = (
  type: TripTypes,
  { travelFrom, travelTo, dateFrom, dateTo, returnDateFrom, returnDateTo },
) => {
  return {
    itinerary: {
      origin: {
